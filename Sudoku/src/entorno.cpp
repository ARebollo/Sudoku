...
bool TEntornoCargarConfiguracion (int   matriz[MAXtamano][MAXtamano],
	...
	std::string cadena;  // para recuperar la informacion de las líneas del fichero
	...
	if (entrada.is_open()) {
		...
		std::cout << "\t[Tamaño del tablero 1..4]" << std::endl;
		std::cout << "\t[Aleatorio(0/se fichero(1))]" << std::endl;
		std::cout << "\t[número de valores aleatórios]" << std::endl;
		std::cout << "\t[matriz de inicio]" << std::endl;
		leido = false;
	}
...
void TEntornoMostrarMensaje(TipoZona zona, std::string msg) {
